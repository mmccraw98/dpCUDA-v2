...
int main(int argc, char** argv) {
    auto [particle, step, console_config, energy_config, state_config, run_config] = load_configs(argc, argv, false);
    ...
    for (double phi = particle->getPackingFraction(); phi <= phi_target; phi += phi_increment) {
        ...
        IOManager nvt_io_manager(nvt_logger_configs, *particle, &nve, sample_dir, 20, overwrite);
        step = 0;
        while (step < num_steps / 10) {
            nve.step();
            nvt_io_manager.log(step);
            step++;
