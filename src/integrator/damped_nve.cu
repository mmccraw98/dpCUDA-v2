...
DampedNVE::DampedNVE(Particle& particle, ConfigDict& config) : Integrator(particle, config), dt(config.at("dt").get<double>()), damping_coefficient(config.at("damping_coefficient").get<double>()) {
}
