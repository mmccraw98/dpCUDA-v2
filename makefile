...
# Specific directories for Particle, Simulator, IO modules
...
EXECUTABLES := $(patsubst $(SCRIPT_DIR)/%.cpp,%,$(SCRIPT_CPP_FILES)) \
               $(patsubst $(TEST_DIR)/%.cpp,%,$(TEST_CPP_FILES))
...
$(EXECUTABLES): %: $(OBJ_DIR)/scripts/%.o $(CU_OBJ_FILES) $(SRC_OBJ_FILES)
...
# Rule for compiling .cu files to object files
